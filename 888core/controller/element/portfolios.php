...
if(!function_exists('tech888f_vc_portfolios')){
    function tech888f_vc_portfolios($attr, $content = false){
        $html = $css_class = '';
        ...
        $el_class .= ' portfolio-'.$display.'-view '.$grid_type.' '.$style;
        ...
        $lst_size_masonry = array();

        if($grid_type == 'masonry-layout'){
            if(!empty($size_mansory_layout)){
                $lst_size_masonry = tech888f_get_size_crop_masonry($size_mansory_layout);
            }
        }
        ...
        $attr = array_merge($attr,array(
            ...
            'args'          => $args,
            'size'          => $size,
            'lst_size_masonry' => $lst_size_masonry
        ));



        $html = tech888f_get_template_element('portfolios/'.$display,$style,$attr);
        wp_reset_postdata();
        return $html;
    }
}

tech888f_reg_shortcode('tech888f_portfolios','tech888f_vc_portfolios');

$check_add = '';

if(isset($_GET['return'])) $check_add = sanitize_text_field($_GET['return']);

if(empty($check_add)) add_action( 'vc_before_init_base','tech888f_add_list_portfolio',10,100 );

if ( ! function_exists( 'tech888f_add_list_portfolio' ) ) {
    function tech888f_add_list_portfolio(){
        vc_map( array(
            "name"      => esc_html__("Portfolios", 'mptheme'),
            "base"      => "tech888f_portfolios",
            "icon"      => "icon-st",
            "category"      => esc_html__("Tech888-Elements", 'mptheme'),
            "description"   => esc_html__( 'Display list of Portfolios', 'mptheme' ),
            "params"    => array(
                array(
                    'type'        => 'textfield',
                    "admin_label"   => true,
                ...
                array(
                    'heading'     => esc_html__( 'Portfolio Categories', 'mptheme' ),
                    'type'        => 'autocomplete',
                    'param_name'  => 'cats',
                    'settings' => array(
                        'multiple' => true,
                        'sortable' => true,
                        'values' => tech888f_get_list_taxonomy('portfolio_category'),
                    ),
                    'save_always' => true,
                    'description' => esc_html__( 'List of Portfolio categories', 'mptheme' ),
                ),
                array(
                    'type' => 'dropdown',
                ...
                array(
                    'heading'       => esc_html__( 'Portfolio style', 'mptheme' ),
                    'type'          => 'dropdown',
                    'description'   => esc_html__( 'Choose style to display.', 'mptheme' ),
                    'param_name'    => 'item_style',
                    'value'         => tech888f_get_portfolio_style(),
                ),
                array(
...
//Load more portfolio
add_action('wp_ajax_load_more_portfolio', 'tech888f_load_more_portfolio');
add_action('wp_ajax_nopriv_load_more_portfolio', 'tech888f_load_more_portfolio');
if (!function_exists('tech888f_load_more_portfolio')) {
    function tech888f_load_more_portfolio()
    {
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                tech888f_get_template_portfolio($style . '/' . $style, $slug, $attr, true);
                $count++;
            }
        }
        wp_reset_postdata();
